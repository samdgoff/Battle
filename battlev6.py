...
def enemyAction(x,enemy):
    ...
    #For debug purposes>
    #print("enemyAction has happened!")
    #<For debug purposes
    damage = 0
    etarget = 0
...
def playSound(filename):
    try:
        playsound("audio/sounds/"+filename+".wav")
    except:
        sleep(0.1)

def bgMusic(stop=False,calm=False):
    if stop == False:
        winsound.PlaySound(None, winsound.SND_PURGE)
        if calm == True:
            filename = "Nice and Calm"
        else:
            filename = getAttrib(eGroupNumber,2,"groups")
        if not filename == "none":
            #I can't find a way to play the intro first, then the rest after, looped.
            #I also need to do this asynchronously. It just won't work without a horrible delay or just being rubbish! GRRRR!!
            #winsound.PlaySound("audio/music/"+filename+"_s.wav", winsound.SND_FILENAME | winsound.SND_ASYNC)
            #winsound.PlaySound("audio/music/"+filename+"_l.wav", winsound.SND_FILENAME | winsound.SND_LOOP | winsound.SND_ASYNC)
            winsound.PlaySound("audio/music/"+filename+".wav", winsound.SND_FILENAME | winsound.SND_LOOP | winsound.SND_ASYNC)
    else:
        winsound.PlaySound(None, winsound.SND_PURGE)
...
while True: #Loops the whole game!
    eGroupNumber = eGroupStart
    ...
    while gameLoop: #Loops the whole game... again?
        battleLoop = True
        eGroupNumber += 1
        groupSetup()
        eDead = []
        ...
        if "~checkpoint~" in getAttrib(4,eGroupNumber,"config"):
            print (getAttrib(1,25,"config"))
            eGroupStart = eGroupNumber-1
            playSound("cheat")
            input()
            print (getAttrib(4,eGroupNumber,"config").replace("~checkpoint~",""))
        else:
        ...
        if health[0] <= 0 and health[1] <= 0: break
    ...
    if gameEnd:
        ...
        while True:
            print()
            print(getAttrib(1,23,"config"))
            tryAgain = input()
            tryAgain = tryAgain.lower()
            if tryAgain == "y" or tryAgain == "yes" or tryAgain == "n" or tryAgain == "no":
                break
            else:
                print (getAttrib(1,4,"config"))
                playSound("error")
        if tryAgain == "y" or tryAgain == "yes":
            print(getAttrib(1,24,"config"))
            playSound("cheat")
            print()
        else:
            break
