...
def bgMusic(stop=False,calm=False):
    if stop == False:
        ...
        if not filename == "none":
            if os.path.isfile("audio/music/"+filename+"_L2.wav"):
                #This if statement would be used for giving music tracks an intro part that gets ignored when it loops,
                #but pygame's queue function for mixer just doesn't work (it's not my coding, it's theirs) and the only
                #other solution is using while loops (so they don't work asynchronously) GRRRRR!!
                mixer.music.load("audio/music/"+filename+"_L2.wav")
            else:
                mixer.music.load("audio/music/"+filename+".wav")
            mixer.music.play(-1)
    else:
        mixer.music.stop()
